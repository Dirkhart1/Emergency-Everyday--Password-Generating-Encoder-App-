...
def number_codes():
  ...
  pass2 = int(list[1])
  pass3 = int(list[2])
  temp = encode(password, pass1, pass2, pass3)
...
password_label.grid(row=0, column=0, sticky=tk.NS)
...
num1_entry.grid(row=1, column=1, ipady=3, ipadx=30, sticky=tk.NS)

# The code below is garbage, I used a different method that doesn't need 2 additonal entrys to help encode the password
# but I left this here for educational purposes (Viewers are welcomed to delete this)
# - - -
"""
num2_code_label = tk.Label(form_frame, text="Num 2")
num2_code_label.grid(row=2, column=2)
num2_entry = tk.Entry(form_frame, validate="key", validatecommand=(root.register(validate_input), '%P'))
num2_entry.grid(row=2, column=3, sticky=tk.E)

num3_code_label = tk.Label(form_frame, text="Num 3")
num3_code_label.grid(row=2, column=4)
num3_entry = tk.Entry(form_frame, validate="key", validatecommand=(root.register(validate_input), '%P'))
num3_entry.grid(row=2, column=5)
"""
# - - -
...
encoded_label.grid(row=3, column=0, sticky=tk.NS)
